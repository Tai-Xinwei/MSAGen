...
class FlashSelfAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        output = (
            ...
            if get_accelerator().device_name() == "cuda"
            else flash_attn_builder.flash_attn_func(
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(
        ...
        self.core_attention = CoreAttention(
            self.layer_number, config, self.attn_mask_type
        )
...
class ParallelTransformerLayerPipe(ParallelTransformerLayer):
    ...
    def forward(self, inputs, **kwargs):
        ...
        if torch.is_tensor(inputs) or len(inputs) == 1:
            ...
            # here we just ignore the moe_loss returned by forward()
            return (
                super().forward(*inputs, **kwargs, rotary_pos_emb=rotary_pos_emb)[0],
                attention_mask,
            )
        else:
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(
        ...
        if self.num_layers == 0:
            ...
            assert (
                len(num_experts) == 1
                or len(num_experts) == args.num_layers // args.expert_interval
