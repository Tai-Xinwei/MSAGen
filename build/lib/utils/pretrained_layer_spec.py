    ...
    def build(self, device="cpu", log=False, load=False):
        ...
        if self.pretrained_ckpt_path is not None and load:
            checkpoints_state = torch.load(
                self.pretrained_ckpt_path, map_location="cpu"
            )
            ...
            if (
                self.pretrained_ckpt_path.split("/")[-1] == "model.hybrid_emb.pt"
                or self.pretrained_ckpt_path.split("/")[-1]
                == "layer_37-model_states.pt"
            ):
                IncompatibleKeys = layer.load_state_dict(
            ...
            if len(IncompatibleKeys["unexpected_keys"]) > 0:
                ds_logger.info(
                    "{} Unexpected keys {}: {}".format(
                        device,
                        self.pretrained_ckpt_path,
                        IncompatibleKeys["unexpected_keys"],
                    )
                )
...
class LoraLayerSpec(LayerSpec):
    def __init__(
        self, typename, ifload=False, lora=True, *module_args, **module_kwargs
    ):
        self.ifload = ifload
        self.pretrained_ckpt_path = module_kwargs.pop("pretrained_ckpt_path", None)
        self.lora = lora
