...
class GraphormerModelPP(nn.Module):
    """
    ...
    def __init__(self, args):
        ...
        self.encoder = GraphormerEncoderPP(args)
...
class GraphormerEncoderPP(nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.pre_sentence_encoder_layer = Pre_sentence_encoder_layer(
        #     # < for graphormer
        #     num_atoms=args.num_atoms,
        #     num_in_degree=args.num_in_degree,
        #     num_out_degree=args.num_out_degree,
        ...
        # droppath_probs = [
        #     x.item() for x in torch.linspace(0, args.droppath_prob, args.encoder_layers)
        # ]

        # for nl in range(args.encoder_layers):
        #     self.g_layers.extend(
        #         [
        #            GraphormerSentenceEncoderLayer_PP(
        #                 embedding_dim=args.encoder_embed_dim,
        #                 ffn_embedding_dim=args.encoder_ffn_embed_dim,
        ...
        # self.post_sentence_encoder_layer = Post_sentence_encoder_layer(args,
        ...
        # self.post_decoder_layer = Post_decoder(args)
...
class Post_decoder(nn.Module):
    def __init__(self, args):
        ...
        if self.load_softmax:
            ...
            if not self.share_input_output_embed:
                ...
                nn.init.uniform_(
                    self.embed_out.weight,
                    -1.0 / (math.sqrt(768.0) * args.d_tilde),
                    1.0 / (math.sqrt(768.0) * args.d_tilde),
                )
            ...
            if args.sent_loss:
                ...
                nn.init.uniform_(
                    self.sentence_projection_layer.weight,
                    -1.0 / (math.sqrt(768.0) * args.d_tilde),
                    1.0 / (math.sqrt(768.0) * args.d_tilde),
                )
        else:
    ...
    def tensors_encode(self, x, node_output):
        shape_tensor = torch.cat(
            [
                torch.tensor(x.contiguous().shape),
                torch.tensor(node_output.contiguous().shape),
            ],
            dim=-1,
        )
