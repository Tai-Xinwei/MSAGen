...
[ -z "${data_path}" ] && data_path=/mnta/yaosen/data/48organisms/48organisms-fullatom.lmdb # /mnta/yaosen/data/esm-test.lmdb  #
[ -z "${task_name}" ] && task_name="subcellular_localization"
[ -z "${loadcheck_path}" ] && loadcheck_path=/home/yaosen/blob/pfmexp/output/toxfold/pfmdiff100M768_prob1261_m5_seq512_x0/global_step14785/mp_rank_00_model_states.pt
[ -z "${save_dir}" ] && save_dir=~/tox_save_dir
...
[ -z "${wandb_group}" ] && wandb_group=bfmdifffold
...
python sfm/tasks/tox/encode_toxfold.py \
    --task_name $task_name \
    --data_path $data_path \
