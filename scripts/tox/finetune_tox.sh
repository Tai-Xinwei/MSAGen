...
[ -z "${log_interval}" ] && log_interval=10000000
[ -z "${epochs}" ] && epochs=100
[ -z "${seed}" ] && seed=42
...
[ -z "${loadcheck_path}" ] && loadcheck_path="/home/peiran/FMproj/data/tox768.pt"
# [ -z "${save_dir}" ] && save_dir="/mnta/yaosen/$task_name"
[ -z "${save_dir}" ] && save_dir="/home/peiran/FMproj/output/$task_name"
[ -z "${early_stopping}" ] && early_stopping=false
[ -z "${early_stopping_patience}" ] && early_stopping_patience=5
[ -z "${early_stopping_metric}" ] && early_stopping_metric='accuracy'
...
torchrun $DISTRIBUTED_ARGS sfm/tasks/tox/finetune_tox.py \
          --task_name $task_name \
