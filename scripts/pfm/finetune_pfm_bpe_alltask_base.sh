...
export loadcheck_path=/blob/shufxi/pfm/bpe/base/lr1.5e-4/checkpoint_E29.pt
...
for task in 'solubility' 'stability' 'EnzymeCommission' 'GeneOntology_mf' 'GeneOntology_bp' 'GeneOntology_cc' 'beta_lactamase' 'fluorescence'  'subcellular_localization' 'subcellular_localization_2' 'remote_homology_fold' 'human_ppi' 'yeast_ppi' 'ppi_affinity'
do
    export task_name=$task
    export WANDB_RUN_NAME="finetune-base_${task_name}_lr${max_lr}_seed${seed}_${run}"
    export save_dir=/blob/shufxi/pfmexp/output/finetune.base.e29/finetune-${task_name}_lr${max_lr}_seed${seed}_${run}
    mkdir -p "$save_dir"
    echo "=============================="
    echo "Task: $task_name"
    echo "=============================="
