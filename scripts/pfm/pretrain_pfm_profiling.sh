...
PROFILING_ARGS=""
PROFILING_ARGS="sudo nsys profile -t cuda,nvtx,osrt,cudnn,cublas,mpi \
     --gpu-metrics-device all -o nsight_report -f true \
     --cuda-memory-usage=true --run-as REDMOND.a-erayinanc"

$PROFILING_ARGS torchrun $DISTRIBUTED_ARGS sfm/tasks/pfm/pretrain_pfm.py \
