...
[ -z "${train_data_path}" ] && train_data_path="/blob/yinxia/wu2/shared/SFM/SFM.overall.data/instruction_tuning/train_signal_peptide.tsv"
[ -z "${val_data_path}" ] && val_data_path="/blob/yinxia/wu2/shared/SFM/SFM.overall.data/instruction_tuning/valid_signal_peptide.tsv"

[ -z "${total_num_epochs}" ] && total_num_epochs=20
...
[ -z "${save_dir}" ] && save_dir="/blob/shufxi/scigpt/7bv3/peptide/$(date +%Y%m%d%H%M%S)"
[ -z "${seed}" ] && seed=46
[ -z "${max_lr}" ] && max_lr=2e-5
[ -z "${beta1}" ] && beta1=0.9
[ -z "${beta2}" ] && beta2=0.999
[ -z "${gradient_accumulation_steps}" ] && gradient_accumulation_steps=32
[ -z "${warmup_num_steps}" ] && warmup_num_steps=400
[ -z "${pipeline_model_parallel_size}" ] && pipeline_model_parallel_size=1
[ -z "${pretrained_ckpt_path}" ] && pretrained_ckpt_path='/hai1/shufxi/scigpt/7bv3/stageB/global_step29999/'
...
torchrun $DISTRIBUTED_ARGS \
    sfm/tasks/scigpt/finetune_scigpt_instruction.py \
