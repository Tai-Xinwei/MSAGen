...
[ -z "${max_lr}" ] && max_lr=5e-6  # LLAMA2 use 3e-4, let's use smaller lr
...
[ -z "${train_data_path}" ] && train_data_path='/blob/renqian/data/sfm/ur90/valid.uniref90.shuf.10k'
[ -z "${valid_data_path}" ] && valid_data_path='/blob/renqian/data/sfm/ur90/valid.uniref90.shuf.10k'

[ -z "${dict_path}" ] && dict_path='/hai1/sfm/llama/llama-2-7b/'
[ -z "${loadcheck_path}" ] && loadcheck_path='/hai1/mfm/shufxi/scigpt/7bv3/stageA_prot_e10_bs512_emb_8xG8H100/global_step5781/'
...
set -x
CUDA_AVAILABLE_DEVICES=0 torchrun sfm/tasks/scigpt/test_scigpt_bio0.py \
