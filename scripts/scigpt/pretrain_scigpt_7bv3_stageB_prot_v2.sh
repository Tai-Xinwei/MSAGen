...
[ -z "${max_lr}" ] && max_lr=1e-4  # LLAMA2 use 3e-4, let's use smaller lr
[ -z "${beta1}" ] && beta1=0.9 # same as LLAMA2
[ -z "${beta2}" ] && beta2=0.95 # same as LLAMA2
[ -z "${total_num_steps}" ] && total_num_steps=191948
[ -z "${warmup_num_steps}" ] && warmup_num_steps=1000
[ -z "${grad_scaler_init}" ] && grad_scaler_init=1

[ -z "${train_batch_size}" ] && train_batch_size=240
[ -z "${val_batch_size}" ] && val_batch_size=240

# When use 96 GPU and pipeline stage 2, the acc steps is 240/ (96/2) = 5
[ -z "${gradient_accumulation_steps}" ] && gradient_accumulation_steps=5
[ -z "${save_epoch_interval}" ] && save_epoch_interval=1
[ -z "${val_batch_interval}" ] && val_batch_interval=1000000
...
[ -z "${train_data_path}" ] && train_data_path='/hai1/shufxi/data/scigpt/v3/train.prot.2.npy'
[ -z "${valid_data_path}" ] && valid_data_path='/hai1/shufxi/data/scigpt/v1/valid_sample.npy'
[ -z "${loadcheck_path}" ] && loadcheck_path='/hai1/shufxi/scigpt/7bv3/stageB.prot/global_step224655'
[ -z "${save_dir}" ] && save_dir='/hai1/shufxi/scigpt/7bv3/stageB.prot.2'
...
# blobfuse bug, !!! not use "-r"
rm /hai1/shufxi/data/scigpt/v3
