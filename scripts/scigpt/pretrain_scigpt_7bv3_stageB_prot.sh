...
[ -z "${total_num_steps}" ] && total_num_steps=264293
[ -z "${warmup_num_steps}" ] && warmup_num_steps=1000
[ -z "${grad_scaler_init}" ] && grad_scaler_init=1

[ -z "${train_batch_size}" ] && train_batch_size=224
[ -z "${val_batch_size}" ] && val_batch_size=224

# When use 224 GPU and pipeline stage 2, the acc steps is 224/ (224/2) = 2
...
[ -z "${loadcheck_path}" ] && loadcheck_path='/hai1/shufxi/scigpt/7bv3/stageB/global_step32999'
[ -z "${save_dir}" ] && save_dir='/hai1/shufxi/scigpt/7bv3/stageB.prot'
