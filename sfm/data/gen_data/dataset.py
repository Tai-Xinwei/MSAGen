from typing import Dict, List, Tuple, Union, Optional, Any, Callable, Iterable, Iterator, TypeVar, Generic
...
class TextMixedWithEntityData(Text):
    ...
    def __init__(self, token_seq: torch.Tensor, entity_id_rage: Dict[EntityType, TokenIdRange]) -> None:
    ...
    def entity_mask(self, entity_type: EntityType) -> torch.Tensor:
        ...
        """
        return (self.token_seq >= self.entity_id_rage[entity_type].start) & (self.token_seq < self.entity_id_rage[entity_type].end)
    ...
    def collate(self, batch: List[TextMixedWithEntityData]) -> TextMixedWithEntityData:
        ...
        for item in batch:
            if item.entity_id_rage != entity_id_rage:
                raise ValueError("The entity_id_rage is not the same for all the texts in the batch.")
        
        # pad the token_seq
        batched_tokens = torch.nn.utils.rnn.pad_sequence([text.token_seq for text in batch], batch_first=True, padding_value=self.pad_idx)
