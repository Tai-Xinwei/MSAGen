...
class InvariantAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            #     key_padding_mask.size(), dtype=torch.bool, device=key_padding_mask.device
            # )
            # local_attention_mask[key_padding_mask <= 1e-5] = True
            # attn_weights = attn_weights.masked_fill(
            #     local_attention_mask.unsqueeze(1).to(torch.bool),
            #     float("-inf"),
            # )
            attn_weights = (
                ((attn_weights + 20.0) * key_padding_mask.unsqueeze(1) - 20.0)
                .float()
                .type_as(q)
            )
        ...
        if key_padding_mask is not None:
            attn_probs_float = (
                torch.mul(attn_probs_float, key_padding_mask.unsqueeze(1))
                .float()
...
class EquivariantAttention(nn.Module):
    def __init__(
        ...
        self.dropout = dropout
        self.attn_ln = EquivariantLayerNorm(hidden_channels, eps=norm_eps)
...
class InvariantSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias, mask, pbc_expand_batched: Optional[Dict] = None):
        ...
        attn = self.invariant_attention(q, k, v, attn_bias, mask, pbc_expand_batched)
...
class EquivariantSelfAttention(nn.Module):
    def __init__(
        ...
        self.equiariant_attention = EquivariantAttention(
            hidden_channels, head_dim, dropout, norm_eps=norm_eps, rank_num=rank_num
        )
    ...
    def forward(self, vec, attn_bias, mask, pbc_expand_batched: Optional[Dict] = None):
        ...
        attn = self.equiariant_attention(q, k, v, attn_bias, mask, pbc_expand_batched)
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.equivariant_ffn_layer_norm_2 = EquivariantLayerNorm(
            ffn_embedding_dim, eps=norm_eps
        )
    ...
    def forward(
        ...
        if self.layer_index % 2 == 0:
            ...
            dvec_invariant = self.equivaiant2invariant_attention(
                dx, dvec, attn_bias_e2i, mask, pos_unit, gbf_args, pbc_expand_batched
            )
