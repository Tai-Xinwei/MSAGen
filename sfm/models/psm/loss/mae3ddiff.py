...
class DiffMAE3dCriterions(nn.Module):
    def __init__(
        ...
        if args.AutoGradForce:
            self.material_force_loss_ratio = 0.9
            self.molecule_force_loss_ratio = 4.9
            self.material_energy_loss_ratio = 0.1
            self.molecule_energy_loss_ratio = 0.1

            logger.info(
                f"overriding force and energy loss ratio in autograd mode:\n {self.material_force_loss_ratio=}, {self.molecule_force_loss_ratio=}, {self.material_energy_loss_ratio=}, {self.molecule_energy_loss_ratio=}"
            )
