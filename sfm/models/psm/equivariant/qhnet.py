...
from .equiformer_v2.layer_norm import EquivariantLayerNormArraySphericalHarmonics
...
class ConvLayer(torch.nn.Module):
    def __init__(
        ...
        if self.use_equi_norm:
            ...
            self.norm = EquivariantLayerNormArraySphericalHarmonics(
                self.lmax, self.irrep_tp_out_node[0][0]
            )
    ...
    def forward(self, data, x):
        ...
        if self.use_equi_norm:
            out = self.norm(out.view(out.shape[0], (self.lmax + 1) ** 2, -1)).view(
                out.shape[0], -1
            )
        ...
        for ins in self.instructions:
            ...
            if ins[3] is True or weights is not None:
                if weights is None:
                    ...
                    result = (
                        torch.einsum("ijk, buvk->buivj", w3j_matrix, result)
                        / mul_ir_in.mul
                    )
...
class QHNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.order == 4:
            ...
            self.hidden_irrep_base = o3.Irreps(
                f"{self.hs}x0e + {self.hs}x1e + {self.hs}x2e"
            )
        elif self.order == 6:
            self.hidden_irrep = o3.Irreps(
                f"{self.hs}x0e + {self.hs}x1o + {self.hs}x2e + {self.hs}x3o + {self.hs}x4e+{self.hs}x5o + {self.hs}x6e"
            )
            self.hidden_irrep_base = o3.Irreps(
                f"{self.hs}x0e + {self.hs}x1e + {self.hs}x2e + {self.hs}x3e + {self.hs}x4e+ {self.hs}x5e + {self.hs}x6e"
            )
        ...
        self.radial_basis_functions = ExponentialBernsteinRadialBasisFunctions(
            self.radius_embed_dim, self.max_radius_cutoff
        )
        self.nonlinear_scalars = {1: "ssp", -1: "tanh"}
        self.nonlinear_gates = {1: "ssp", -1: "abs"}
        self.num_fc_layer = 1
        ...
        (
            ...
            self.fc_ii_bias,
            self.fc_ij_bias,
        ) = [nn.ModuleDict() for _ in range(6)]
    ...
    def forward(self, data, return_full_hami=False):
        ...
        data.ptr = torch.tensor(
            [
                data["molecule_size"][:i].sum()
                for i in range(data["molecule_size"].shape[0] + 1)
            ]
        ).to(self.device)

        node_attr, edge_index, rbf_new, edge_sh, _ = self.build_graph(
            data, self.max_radius_cutoff
        )
    ...
    def build_graph(self, data, max_radius):
        ...
        radius_edges = radius_graph(
            data.pos, max_radius, data.batch, max_num_neighbors=1000
        )
                ...
                for dst_idx in range(data.ptr[graph_idx], data.ptr[graph_idx + 1]):
                    if src_idx == dst_idx:
                        matrix_col.append(
                            ...
                            .index_select(
                                -2,
                                self.orbital_mask[data.atomic_numbers[dst_idx].item()],
                            )
                            .index_select(
                                -1,
                                self.orbital_mask[data.atomic_numbers[src_idx].item()],
                            )
                        )
                    else:
                        mask1 = src == src_idx
...
class QHNet_backbone(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        (
            ...
            full_edge_sh,
            transpose_edge_index,
        ) = self.build_graph(batch_data, 10000)
        (
            batch_data.full_edge_index,
            batch_data.full_edge_attr,
            batch_data.full_edge_sh,
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            if layer_idx > self.start_layer:
                fii = self.e3_gnn_node_layer[layer_idx - self.start_layer - 1](
                    batch_data, node_attr, fii
                )
                fij = self.e3_gnn_node_pair_layer[layer_idx - self.start_layer - 1](
                    batch_data, node_attr, fij
                )
        fii = self.output_ii(fii)
        fij = self.output_ij(fij)
