...
class VecLayerNorm(torch.nn.Module):
    ...
    def forward(self, vec: Tensor) -> Tensor:
        ...
        if vec.size(1) == 3:
            ...
            vec1, vec2, vec3, vec4, vec5, vec6 = torch.split(
                vec, [3, 5, 7, 9, 11, 13], dim=1
            )
            if self.norm_type == "max_min":
                vec1 = self.max_min_norm(vec1)
                vec2 = self.max_min_norm(vec2)
            ...
            vec1, vec2, vec3, vec4, vec5, vec6, vec7 = torch.split(
                vec, [3, 5, 7, 9, 11, 13, 15], dim=1
            )
            if self.norm_type == "max_min":
                vec1 = self.max_min_norm(vec1)
                vec2 = self.max_min_norm(vec2)
            ...
            vec1, vec2, vec3, vec4, vec5, vec6, vec7, vec8 = torch.split(
                vec, [3, 5, 7, 9, 11, 13, 15, 17], dim=1
            )
            if self.norm_type == "max_min":
                vec1 = self.max_min_norm(vec1)
                vec2 = self.max_min_norm(vec2)
            ...
            vec1, vec2, vec3, vec4, vec5, vec6, vec7, vec8, vec9 = torch.split(
                vec, [3, 5, 7, 9, 11, 13, 15, 17, 19], dim=1
            )
            if self.norm_type == "max_min":
                vec1 = self.max_min_norm(vec1)
                vec2 = self.max_min_norm(vec2)
            ...
            vec = torch.cat(
                [vec1, vec2, vec3, vec4, vec5, vec6, vec7, vec8, vec9], dim=1
            )
            return vec * self.weight.unsqueeze(0).unsqueeze(0)
        else:
...
class ViSNet(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        data["natoms"] = scatter(
            torch.ones_like(data.batch), data.batch, dim=0, reduce="sum"
        )
