...
from sfm.models.pfm.openfold.primitives import LayerNorm, Linear

# from sfm.models.pfm.openfold.utils.precision_utils import is_fp16_enabled
...
class AuxiliaryHeads(nn.Module):
    def __init__(self, config):
        ...
        self.distogram = DistogramHead(
            **vars(config.distogram),
        )

        # self.masked_msa = MaskedMSAHead(
        #     **vars(config.masked_msa),
        # )

        self.experimentally_resolved = ExperimentallyResolvedHead(
            **vars(config.experimentally_resolved),
        )
