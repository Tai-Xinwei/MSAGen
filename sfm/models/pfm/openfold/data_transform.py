...
from sfm.models.pfm.openfold import residue_constants as rc
...
from sfm.models.pfm.openfold.rigid_utils import Rigid, Rotation
from sfm.models.pfm.openfold.tensor_utils import (
    batched_gather,
    tensor_tree_map,
    tree_map,
)
...
def esm_to_alphafold_aatype(protein):
    esm_aa = protein["x"].clone()
    B, L = esm_aa.shape
    # cls_idx = esm_aa == 0
    # eos_idx = esm_aa == 2
    # # rm eos and cls from esm_aa
    # esm_aa = esm_aa[~(cls_idx | eos_idx)].view(B, L-2)
    protein["x"] = esm_aa
    mapping_tensor = torch.tensor(
        [
            rc.ESM_TO_ALPHAFOLD[i] if i in rc.ESM_TO_ALPHAFOLD else i
            for i in range(max(rc.ESM_TO_ALPHAFOLD.keys()) + 1)
        ],
        device=esm_aa.device,
    )
    # map esm_aa to alphafold aatype
    protein["aatype"] = mapping_tensor.index_select(0, esm_aa.view(-1)).view(B, L)
    # pos and pos_mask has shape [B, L, 37, 3], rm eos and cls from pos and pos_mask
    pos = protein["pos"].clone()
    pos_mask = protein["pos_mask"].clone()

    # pos = pos[~(cls_idx | eos_idx)].view(B, L-2, 37, 3)
    # pos_mask = pos_mask[~(cls_idx | eos_idx)].view(B, L-2, 37)

    protein["all_atom_positions"] = pos
    protein["all_atom_mask"] = pos_mask
    ...
    protein["seq_mask"] = (~padding_mask).float()
...
def make_seq_mask(protein):
    protein["seq_mask"] = torch.ones(
        protein["aatype"].shape, dtype=torch.float32, device=protein["aatype"].device
    )
    return protein
