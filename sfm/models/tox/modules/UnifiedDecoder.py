...
class UnifiedDecoder(nn.Module):
    def __init__(
        ...
        self.angle_proj = nn.Linear(3, embedding_dim, bias=False)
        ...
        self.time_embedding = TimeStepEncoder(
            n_timesteps=1010,
            timestep_emb_dim=embedding_dim,
    ...
    def forward(
        ...
        #     time_embedding = (
        #         torch.zeros_like(uni_gbf_feature).to(uni_gbf_feature) + t_dist
        #     )
        ...
        #     t0_emb = torch.zeros_like(uni_gbf_feature).to(uni_gbf_feature) + t0_dist
        ...
        #     uni_gbf_feature = uni_gbf_feature + time_embedding
        ...
        #     angle = angle.to(x.dtype) / 180.0
        #     angle_feat = (
        #         self.angle_proj(angle.masked_fill(angle_mask, 0.0)) + time_embedding
        #     )

        #     output = x.contiguous().transpose(0, 1)[:, :, :] + angle_feat
        # else:
        ...
        if time_pos is not None and mask_pos is not None:
            ...
            output = output + time_embedding_pos
        ...
        pos_output = self.unified_node_output_layer(output, node_output).squeeze(-1)
        # angle_output = self.unified_angle_output_layer(output, node_output).squeeze(-1)
        angle_output = self.unified_angle_output_layer(output)

        pos_output = pos_output.masked_fill(padding_mask[:, :].unsqueeze(-1), 0.0)
        angle_output = angle_output.masked_fill(padding_mask[:, :].unsqueeze(-1), 0.0)

        return pos_output, angle_output, output
