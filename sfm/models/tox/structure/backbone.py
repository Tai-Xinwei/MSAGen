...
from sfm.models.tox.structure import geometry, transforms
...
class BackboneBuilderV0(nn.Module):
    ...
    def __init__(self):
        super(BackboneBuilderV0, self).__init__()
    ...
    def forward(
        ...
        if lengths is None:
            ...
            # ).to(device)
            lengths = torch.tensor(
                [[self.lengths[key] for key in ["N_CA", "CA_C", "C_N"]]],
                dtype=torch.float32,
            ).to(device)
        ...
        if angles is None:
            ...
            # ).to(device)
            angles = torch.tensor(
                [[self.angles[key] for key in ["C_N_CA", "N_CA_C", "CA_C_N"]]],
                dtype=torch.float32,
            ).to(device)
        ...
        dihedrals = torch.stack([omega, phi, psi], -1)
...
class BackboneBuilder(BackboneBuilderV0):
    """Protein backbone builder from dihedral angles (batch form).
    ...
    def __init__(self):
        ...
        self.proj_angle = nn.Sequential(
            nn.Linear(1, 32, bias=False),
            nn.GELU(),
            nn.Linear(32, 64, bias=False),
        )
        self.lstm = torch.nn.LSTM(64, 64, 2, batch_first=True, bidirectional=True)
    ...
    def forward(
        ...
        dihedrals = torch.stack([omega, phi, psi], -1)
        dihedrals = dihedrals.view([N_batch, 3 * N_residues, 1])

        if add_O:
            offset = dihedrals[:, -3:, :]
            dihedrals = torch.cat([dihedrals, offset], 1)

        X = self.proj_angle(dihedrals)
        X = self.lstm(X)[0]
        X = self.proj_pos(X).view([N_batch, -1, 3, 3])
