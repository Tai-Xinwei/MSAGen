...
class InternalCoords(nn.Module):
    ...
    def forward(
        ...
        def _lengths(Xi, Xj):
            return lengths(Xi, Xj, distance_eps=self.distance_eps)

        def _angles(Xi, Xj, Xk):
            return np.pi - angles(Xi, Xj, Xk, distance_eps=self.distance_eps)

        def _dihedrals(Xi, Xj, Xk, Xl):
            return dihedrals(Xi, Xj, Xk, Xl, distance_eps=self.distance_eps)
...
def quaternions_from_rotations(R: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
    ...
    def _sqrt(r):
        return torch.sqrt(F.relu(r) + eps)
