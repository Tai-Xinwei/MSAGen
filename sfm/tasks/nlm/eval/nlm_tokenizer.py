...
import numpy as np
import torch
from science_tokens import SCIENCE_TAG_TOKENS, SCIENCE_TOKENS
...
class NlmTokenizer(LlamaTokenizer):
    def __init__(self, *args, **kwargs):
        ...
        print(f"Tokenizer has {len(self)} tokens")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_dna_six:
            ...
            for i in range(len(all_kmers)):
                extra_tokens.append(f"<d>{all_kmers[i]}")
                self.dna_six_encode_dict[all_kmers[i]] = True
            print("Tokenizer use DNA six mer")
        self.add_tokens(extra_tokens)
        self.split_special_tokens = True  # Ensure _tokenize() can access special tokens
...
if __name__ == "__main__":
    mixtral_home = "mistralai/Mixtral-8x7B-Instruct-v0.1"
    tokenizer = NlmTokenizer.from_pretrained(mixtral_home)
    print("vocab size", len(tokenizer))
    print("eos token id", tokenizer.eos_token_id)
    print("eos token", tokenizer.decode(tokenizer.eos_token_id))

    print("Yes id", tokenizer("Yes").input_ids)
    print("▁Yes id", tokenizer("▁Yes").input_ids)
    print("Decoded Yes id", tokenizer.decode(tokenizer("Yes").input_ids))
    print("Decoded ▁Yes id", tokenizer.decode(tokenizer("▁Yes").input_ids))
    ...
    prompt_token_ids = [tokenizer(prompt).input_ids for prompt in prompts]
