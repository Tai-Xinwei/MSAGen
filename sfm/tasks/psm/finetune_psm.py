...
def load_data(args, extra_collate_fn=None):
    ...
    for data_path, dataset_name in zip(file_list, dataset_name_list):
        ...
        train_dataset, valid_dataset = dataset.split_dataset(
            training_ratio=0.9, validation_ratio=0.1
        )
...
@hydra.main(
    ...
    config_name="config_psm_debug",
)
def finetune(cfg: DictConfig) -> None:
    args = OmegaConf.to_object(cfg)
    # with open("final_config_1.yaml", "w") as f:
    ...
    if args.wandb_run_name == "":
        args.wandb_run_name = f"{args.backbone}_N@{args.encoder_layers}_D@{args.encoder_embed_dim}_DS@{args.dataset_name_list}"
    else:
        args.wandb_run_name = f"{args.wandb_run_name}"
    args.save_dir = os.path.join(args.save_dir, args.wandb_project, args.wandb_run_name)
    ...
    train_data, valid_data = load_data(args, extra_collate_fn=extra_collate_fn)
