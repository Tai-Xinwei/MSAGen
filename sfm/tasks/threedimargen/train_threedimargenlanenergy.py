...
def main(args) -> None:
    ...
    assert config.tokenizer in ["lan", "lanv2"]
    ...
    tokenizer = ThreeDimARGenEnergyTokenizer.from_file(config.dict_path, config)
    config.vocab_size = len(tokenizer)
    config.pad_token_id = tokenizer.padding_idx

    train_dataset = ThreeDimARGenEnergyDataset(
    ...
    if config.valid_data_path is not None and len(config.valid_data_path) > 0:
        valid_dataset = ThreeDimARGenEnergyDataset(
