    ...
    with lmdb.open(lmdbdir, readonly=True).begin(write=False) as txn:
        ...
        for key in selected_keys:
            ...
            for chainid, chain in data['polymer_chains'].items():
                mask = np.any(~np.isnan(chain["allatom_coord"]), axis=-1)
                polypos.append(chain["allatom_coord"][mask])
            polypos = np.concatenate(polypos, axis=0)

            for graph in data['nonpoly_graphs']:
                mask = ~np.any(np.isnan(graph['node_coord']), axis=-1)
                ligpos = graph['node_coord'][mask]
                dist = np.linalg.norm(ligpos[None, :] - polypos[:, None], axis=-1)
                print(f"{data['pdbid']}_{graph['chain_id']}_{graph['name']}", np.min(dist))
