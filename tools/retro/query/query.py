...
def get_index(ondisk=False):
    ...
    faiss.ParameterSpace().set_index_parameter(
        index, "efSearch", args.retro_query_ef_search
    )
...
def query_embeddings(
    ...
    _, query_neighbor_ids = index.search(
        embeddings, args.retro_query_num_neighbors_query
    )
    if verbose:
...
def query_dataset_neighbors(
    db_dataset, query_dataset, prefix, neighbor_dir, index, embedder
):
