    ...
    with ProcessPoolExecutor(max_workers=n_procs) as executor:
        for block_idx, block in enumerate(missing_db_blocks):
            if block is not None:
                ...
                doc_offsets = np.cumsum([item[1] for item in doc_sizes]).astype(
                    "uint64"
                )
...
def update_chunk_counts(indexed_dataset_infos):
    ...
    for ds_index, ds_info in enumerate(indexed_dataset_infos):
        ...
        for db_path in tqdm(
            db_paths,
            "%d/%d, %s" % (ds_index, len(indexed_dataset_infos), ds_info["name"]),
        ):
...
def merge_dbs(indexed_dataset_infos, db_type):
    ...
    if db_type == "valid":
        ...
        n_docs = (
            ...
            else sum(m[n_docs_key] for m in indexed_dataset_infos)
        )
    ...
    if not os.path.exists(db_path):
        ...
        for ds_idx, ds_info in enumerate(indexed_dataset_infos):
            ...
            individual_doc_offsets = (
                None
                if n_docs_key is None
                else get_individual_doc_offsets(ds_idx, ds_info)
            )
            ...
            if db_type == "valid":
                ...
                if n_docs_key is None:
                    ...
                    train_doc_offset = individual_doc_offsets[
                        ds_info["n_docs_train"] - 1, 2
                    ]
