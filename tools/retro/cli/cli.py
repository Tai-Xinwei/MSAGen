...
class retro:
    ...
    def init(cls, workdir):
        ...
        for key in vars(cls.pt_datasets):
            getattr(
                cls.pt_datasets, key
            ).num_neighbors = cls.args.retro_query_num_neighbors_save
    ...
    def get_neighbor_tokens(cls, sample_id, chunk_id, data_key="train"):
        try:
            ...
            chunk_end_idx = min(
                sample_token_ids.shape[0], chunk_start_idx + chunk_length
            )
    ...
    def print_usage(cls):
        ...
        print(
            ...
            % shorten_str(
                str(sample["neighbor_tokens"][sample_chunk_id][sample_neighbor_id]), 50
            )
        )
        print(
        ...
        print(
            ...
            % shorten_str(
                cls.gpt_to_text(
                    sample["neighbor_tokens"][sample_chunk_id][sample_neighbor_id]
                ),
                50,
            )
        )
