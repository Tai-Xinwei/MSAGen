...
class MAE3d_criterions(nn.Module):
    ...
    def forward(self, batch_data, logits, node_output):
        ...
        pos_loss = self.l2(node_output.to(torch.float32), ori_pos.to(torch.float32)).sum(dim=-1) * self.args.pos_loss_coeff
...
class MAE3d_criterions_PP(nn.Module):
    ...
    def forward(self, output, label):
        ...
        loss1 = self.l1(logits.view(-1, logits.size(-1)).to(torch.float32),
                        targets.view(-1), ) * self.args.atom_loss_coeff

        node_output_loss = self.l2(node_output.to(torch.float32), 
                                ori_pos.to(torch.float32)).sum(dim=-1) * self.args.pos_loss_coeff
