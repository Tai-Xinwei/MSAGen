...
class L1_criterions(nn.Module):
    ...
    def forward(self, batch_data, logits, node_output):
        ...
        loss = self.l1(logits, (y[: logits.size(0)] - self.datasetmean) / self.datasetstd)
...
class Binary_criterions(nn.Module):
    ...
    def forward(self, output, label):
        ...
        # print(y.shape, logits.shape)
        # print(y, logits) ; exit()
        loss = nn.functional.binary_cross_entropy_with_logits(logits.reshape(-1).float(), y.reshape(-1).float(), reduction=self.reduction)
        # loss = self.l1(logits, y[: logits.size(0)])
