...
class GraphPredictionLoss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        node_output_loss = nn.L1Loss(reduction='sum')(node_output.to(torch.float32), ori_pos.to(torch.float32)).sum(dim=-1) * self.pos_loss_coeff
...
@register_criterion("shortest_path_distance_prediction", dataclass=GraphPredictionConfig)
...
class GraphPredictionMultiTaskLoss(FairseqCriterion):
    ...
    loss_task_mapping = {
        v:k for k, v in task_loss_mapping.items()
    }
    ...
    def __init__(self, cfg: GraphPredictionConfig, task):
        ...
        self.classification_tasks_list = getattr(task.cfg, 'classification_tasks_list', None)
        assert self.regression_tasks_list != None, "Must set regression tasks list"
        assert self.classification_tasks_list != None, "Must set classification tasks list"
        if self.regression_tasks_list == 'None' and self.classification_tasks_list == 'None':
        ...
        self.regression_tasks_list = [item for item in self.regression_tasks_list if item != 'None']
        self.classification_tasks_list = [item for item in self.classification_tasks_list if item != 'None']
        for item in self.regression_tasks_list + self.classification_tasks_list:
            assert item in self.loss_task_mapping.keys(), f"regression/classification task names must be in the supported list {self.loss_task_mapping.keys()}"
    ...
    def forward(self, model, sample, reduce=True):
        ...
        for i in range(self.regression_num_tasks):
            cur_regression_task_idx = self.loss_task_mapping[self.regression_tasks_list[i]]
            cur_loss = nn.L1Loss(reduction='sum')(logits[i].squeeze(), targets[:, cur_regression_task_idx])
            loss_logging_output[self.regression_tasks_list[i]] = cur_loss.data
            loss = loss + cur_loss / (self.regression_num_tasks + self.classification_num_tasks)
        for i in range(self.classification_num_tasks):
            cur_classification_task_idx = self.loss_task_mapping[self.classification_tasks_list[i]]
            cur_loss = nn.CrossEntropyLoss(reduction='sum')(logits[i + self.regression_num_tasks], targets[:, cur_classification_task_idx].long())
