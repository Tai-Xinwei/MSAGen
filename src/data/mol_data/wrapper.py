...
class PM6FullLMDBDataset(InMemoryDataset):
    def __init__(
        ...
        self.env_list = [
            lmdb.Environment(
                ...
                readahead=False,
                meminit=False,
            ).begin() for item in self.db_path_list
        ]
    ...
    def get_keys_list(self):
        ...
        key_dir_list = [osp.join(item, 'key_list.pt') for item in self.smiles_db_path_list]
        key_list = []
        for i, item in enumerate(key_dir_list):
            ...
            print(f'Loaded key_list for {item.split("/")[-2]}; time: {end_time - begin_time} s')
        return key_list
    ...
    def smiles_db_dir(self) -> List[str]:
        return [
            osp.join(osp.join(f'{self.root}', 'merged'), 'S0-smiles')
        ]

    @property
    def raw_file_names(self) -> List[str]:
        return [
            osp.join(osp.join(osp.join(f"{self.root}", "merged"), "S0-msg"), "data.mdb"),
