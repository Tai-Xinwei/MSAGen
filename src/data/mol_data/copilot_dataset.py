...
# TODO (Roger)
def _tokenize_fn_moleculenet(strings: Sequence[str], 
                             tokenizer: transformers.PreTrainedTokenizer, 
                             smiles: Sequence[str], 
                             smiles_dict: Dict, 
                             nnodes: Sequence[int], 
                             pool_mode: str = 'cls', 
                             embedding_length: int = 1) -> Dict:
    ...
    for idx, text in enumerate(strings):
        ...
        if pool_mode == 'full':
            ...
            input_id = torch.cat([split_tokenized_list[0].input_ids[0], to_replace_list, split_tokenized_list[-1].input_ids[0]])
            ...
            input_id = torch.cat([split_tokenized_list[0].input_ids[0], to_replace_id, split_tokenized_list[-1].input_ids[0]])
...
def preprocess_mask(tokenizer: transformers.PreTrainedTokenizer, input_ids: torch.Tensor, labels: torch.Tensor):
    input_ids = torch.nn.utils.rnn.pad_sequence(
...
def preprocess_moleculenet(
    ...
    examples_tokenized, sources_tokenized = [_tokenize_fn_moleculenet(strings, tokenizer, smiles, smiles_dict, nnodes, pool_mode=pool_mode, embedding_length=embedding_length) for strings in (examples, sources)]
...
class SupervisedMoleculeNetDataset(Dataset):
    ...
    def __init__(self, data_path: str, dataset_names: str, smiles_dict_path: str, tokenizer: transformers.PreTrainedTokenizer, mode='train', pool_mode='cls', embedding_length=1):
        # data_path: ./data/
        # dataset_name: hiv
        ...
        for example in list_data_dict:
            ...
            cur_smiles = example.get("entities", "").get("<<|mol0|>>", "").get("smiles", "")
        ...
        data_dict = preprocess_moleculenet(sources, targets, entity_replace_smiles, smiles_dict, tokenizer, nnodes, pool_mode=pool_mode, embedding_length=embedding_length)

        input_ids = data_dict["input_ids"]
        labels = data_dict["labels"]
        self.input_ids, self.labels, self.llm_mask = preprocess_mask(tokenizer, input_ids, labels)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        item = preprocess(self.input_ids[i], self.labels[i], self.llm_mask[i], self.data[i], i)
        return item
...
class SupervisedProcessedData(Dataset):
    def __init__(self, data_path: str, mol_size_path: str, mol2idx_dict_path: str,
                 dataset_names: str, dataset_splits: str, in_memory: bool, pad_token_id: int,
                 pool_mode: str, embedding_length: int = 1, dataset_ratios: str = None) -> None:
        super().__init__()
        self.data_path = data_path
        self.dataset_names = dataset_names.split(",")
        self.dataset_ratios = None if dataset_ratios is None else [float(ratio) for ratio in dataset_ratios.split(",")]
        self.dataset_splits = [dataset_split.split("+") for dataset_split in dataset_splits.split(",")]
        assert len(self.dataset_splits) == len(self.dataset_names), f"Lengths of dataset_splits and dataset_names do not match."
        assert self.dataset_ratios is None or len(self.dataset_ratios) == len(self.dataset_names), f"Lengths of dataset_ratios and dataset_names do not match."
        ...
        if not self.in_memory:
            for i, (dataset_name, dataset_splits) in enumerate(zip(self.dataset_names, self.dataset_splits)):
                ...
                for dataset_split in dataset_splits:
                    logging.warning(f"Loading dataset {dataset_name} split {dataset_split}")
                    read_env = lmdb.open(f"{self.data_path}/{dataset_name}/{dataset_split}/")
                    ...
                    len_read_env = lmdb.open(f"{self.data_path}/{dataset_name}/{dataset_split}-len/")
                    ...
                    for key, val in read_txn.cursor():
                        ...
                        assert len(mol_idxs) == len(val[val < 0]), f"{mol_idxs} vs. {val}"
                        self.index_to_key_map.append((dataset_name, dataset_split, key.decode()))
        ...
        for dataset_name in self.dataset_count:
            for dataset_split in self.dataset_count[dataset_name]:
                logging.warning(f"Dataset {dataset_name} split {dataset_split}: {self.dataset_count[dataset_name][dataset_split]} loaded, {self.dataset_filtered[dataset_name][dataset_split]} filtered.")
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        if not self.in_memory:
            input_ids, input_ids_len = pkl.loads(self.read_txns[dataset_name][dataset_split].get(key.encode()))
        else:
            input_ids, input_ids_len = self.in_memory_data[dataset_name][dataset_split][key]
        ...
        mol_idx_pos = torch.cat([torch.tensor([-1]), mol_idx_pos, torch.tensor([len(input_ids)])])

        assert self.mol_data_offset[index + 1] - self.mol_data_offset[index] == len(mol_idxs), f"{input_ids} vs. {key} vs. {self.mol_data_offset[index + 1] - self.mol_data_offset[index]} vs. {len(mol_idxs)}, {mol_idxs}"

        if self.pool_mode == 'full' or self.pool_mode == 'qformer' or self.pool_mode == 'multimol':
            input_ids_expanded = []
            for i in range(len(mol_idx_pos) - 1):
                ...
                if cur_pos >= 0 and cur_pos < len(input_ids):
                    ...
                    if self.pool_mode == 'full':
                        input_ids_expanded.append(torch.tensor([mol_idx for _ in range(mol_size)]))
                    elif self.pool_mode == 'qformer' or self.pool_mode == 'multimol':
                        # input_ids_expanded.append(torch.tensor([1]))
                        input_ids_expanded.append(torch.tensor([mol_idx for _ in range(self.embedding_length)]))
                        # input_ids_expanded.append(torch.tensor([1]))
                input_ids_expanded.append(input_ids[cur_pos + 1: mol_idx_pos[i + 1]])

            input_ids = torch.cat(input_ids_expanded)
        else:
            raise Exception(f"Unknown pool mode {self.pool_mode}, should be full, qformer or multimol")

        processed_mol_data = []
        for j, mol_data_idx in enumerate(range(self.mol_data_offset[index], self.mol_data_offset[index + 1])):
        ...
        return dict(input_ids=input_ids, labels=labels, llm_mask=input_ids.ne(self.pad_token_id), processed_mol_data=processed_mol_data)
