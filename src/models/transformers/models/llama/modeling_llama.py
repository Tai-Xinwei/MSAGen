...
class MLPAdapter(nn.Module):
    def __init__(
        ...
        # self.up_proj = nn.Linear(hidden_size, intermediate_size, bias=False)
        self.ln = nn.LayerNorm(intermediate_size)
        self.act_fn = ACT2FN[hidden_act]

    def forward(self, x):
        return self.down_proj(self.ln(self.act_fn(self.gate_proj(x))))
...
class LlamaMLPAdapter(nn.Module):
    ...
    def forward(self, x):
        return self.down_proj(self.act_fn(self.dropout(self.gate_proj(x)) * self.up_proj(x)))
...
class EmbedAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_attention_heads: int, dropout: float = 0.0):
        super().__init__()
        # self.config = config
    ...
    def forward(
        ...
        if bsz != bszq:
            query_states = self.q_proj(q_states).view(1, -1, self.num_heads, self.head_dim).expand(bsz, -1, -1, -1).transpose(1, 2)
        else:
            query_states = self.q_proj(q_states).view(bsz, -1, self.num_heads, self.head_dim).transpose(1, 2)

        key_states = self.k_proj(hidden_states).view(bsz, kv_seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(bsz, kv_seq_len, self.num_heads, self.head_dim).transpose(1, 2)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.mol_adapter = LlamaMLPAdapter(hidden_size=config.mfm_hidden_size, intermediate_size=config.mfm_hidden_size,
            ...
            for tidx in range(T):
                if mol_idx_mask[bidx, tidx]:
                    ...
                    if mol_rep is None:
                        ...
                        mol_rep = torch.zeros([3, self.config.mfm_hidden_size]).to(dtype=token_embed.dtype, device=token_embed.device)
                    else:
                        mol_rep = pkl.loads(mol_rep).to(dtype=token_embed.dtype, device=token_embed.device)
                    token_embed[bidx, tidx] = self.mol_adapter(mol_rep)[1:, :].mean(dim=0)
        return token_embed
...
class hybrid_emb(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.mol_rep_layernorm = LlamaRMSNorm(config.mfm_hidden_size, eps=config.rms_norm_eps)
        if config.btn_adaptor:
            self.mol_adapter = LlamaMLPAdapter(hidden_size=config.mfm_hidden_size, intermediate_size=config.mfm_hidden_size//4,
                                            output_size=config.hidden_size, hidden_act=config.hidden_act)
        else:
            self.mol_adapter = LlamaMLPAdapter(hidden_size=config.mfm_hidden_size, intermediate_size=config.mfm_hidden_size,
                                            output_size=config.hidden_size, hidden_act=config.hidden_act, dropout=config.hidden_dropout_prob)
        
        if self.mode == 'qformer' or self.mode == 'multimol':
            self.embed_attn = EmbedAttention(hidden_size=config.mfm_hidden_size, num_attention_heads=32, dropout=config.hidden_dropout_prob)
            self.cross_attn = EmbedAttention(hidden_size=config.mfm_hidden_size, num_attention_heads=32, dropout=config.hidden_dropout_prob)
            self.adaptor_embedding = torch.zeros((1, config.embedding_length, config.mfm_hidden_size), requires_grad=True)
            self.embedding_length = config.embedding_length
    ...
    def _forward_embedding(self, mol_rep, padding_mask, input_ids: torch.LongTensor = None):
        # TODO (Roger)
        # input_ids: bsz, num_tokens
        B, T = input_ids.shape
        mol_idx_mask = input_ids < 0 # B, T
        ...
        with torch.no_grad():
            token_embed = self.embed_tokens(input_ids.masked_fill(mol_idx_mask, 0)) # B, T, hidden_size
        ...
        if self.mode == "mean":
            pooled_rep = self.mol_adapter(mol_rep) * (1 - padding_mask) # padding_mask: B, nnodes
            ...
            mol_idx_mask = mol_idx_mask.unsqueeze(-1).expand(-1, -1, self.config.hidden_size) # B, T, H
            token_embed = torch.where(mol_idx_mask, pooled_rep, token_embed)
        elif self.mode == "cls":
            pooled_rep = self.mol_adapter(mol_rep)[:, 0, :].unsqueeze(1).expand(-1, T, -1) # B, T, H
            mol_idx_mask = mol_idx_mask.unsqueeze(-1).expand(-1, -1, self.config.hidden_size) # B, T, H
            token_embed = torch.where(mol_idx_mask, pooled_rep, token_embed)
        elif self.mode == "full":
            ...
            for bidx in range(B):
                mask_idx_list = (mol_idx_mask[bidx] == True).nonzero()
                start = mask_idx_list[0]
                end = mask_idx_list[-1]
                # print(mol_idx_mask, start, end); exit()
                token_embed[bidx, start:end+1, :] = mol_rep[bidx, :end-start+1, :]
        elif self.mode == "qformer":
            self.adaptor_embedding = self.adaptor_embedding.to(mol_rep.device).to(mol_rep.dtype)
            embedding_f, _, _ = self.embed_attn(hidden_states=self.adaptor_embedding, q_states=self.adaptor_embedding)

            # mol_rep = mol_rep[:, 1:, :]  # B, nnode, H
            attn_mask = padding_mask.unsqueeze(1).unsqueeze(2).squeeze(-1).expand(-1, -1, self.embedding_length, -1) # B, 1, L, nnodes
            embed_mol_rep, _, _ = self.cross_attn(hidden_states=mol_rep, q_states=embedding_f, attention_mask=attn_mask)
    ...
    def forward(self, input_tuple: Tuple):
        ...
        # attention_mask = torch.where(attention_mask < 0, -100000, 0).long()
        # print(attention_mask); exit()

        hidden_states = inputs_embeds
        # print("hybrid_emb:", hidden_states.grad, attention_mask.grad, position_ids.grad)
        # print("hybrid_emb:", hidden_states.dtype, attention_mask.dtype, position_ids.dtype)
...
class LlamaModel_PP(LlamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaDecoderLayer`]
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = []
        # self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        # self.hybemb = hybrid_emb(config)
        # self.layers.append(self.hybemb)
        self.layers.append(LayerSpec(hybrid_emb, config))
        # self.layers = nn.ModuleList([LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        # self.layers.extend([LlamaDecoderLayerPP(config) for _ in range(config.num_hidden_layers)])
        self.layers.extend([LayerSpec(LlamaDecoderLayerPP, config)] for _ in range(config.num_hidden_layers))

        # self.classifier = LlamaClassifier(config)
...
class LlamaForCausalLM_PP(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        # self.layers.append(self.lm_head)
