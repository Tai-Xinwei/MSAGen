...
class GraphNodeFeature(nn.Module):
    ...
    def __init__(self, args, num_heads, num_atoms, num_in_degree, num_out_degree, hidden_dim, n_layers, no_2d=False):
...
class GraphAttnBias(nn.Module):
    ...
    def __init__(self, args, num_heads, num_atoms, num_edges, num_spatial, num_edge_dis, hidden_dim, edge_type, multi_hop_max_dist, n_layers, no_2d=False):
...
class Graph3DBias(nn.Module):
    ...
    def __init__(self, args, num_heads, num_edges, n_layers, embed_dim, num_kernel, no_share_rpe=False):
...
class NodeTaskHead(nn.Module):
    def __init__(
        ...
        self.force_proj1: Callable[[Tensor], Tensor] = nn.Linear(embed_dim, 1, bias=False)
...
class GatedEquivariantBlock(nn.Module):
    ...
    def reset_parameters(self, d_tilde):
        ...
        nn.init.xavier_uniform_(self.update_net[0].weight, gain=1.0 / math.sqrt(d_tilde))
        if self.out_channels == 1:
            ...
            nn.init.xavier_uniform_(self.vec2_proj.weight, gain=1.0 / math.sqrt(d_tilde))
            nn.init.xavier_uniform_(self.update_net[2].weight, gain=1.0 / math.sqrt(d_tilde))
