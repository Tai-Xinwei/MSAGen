...
from .myPipelineParallelGrid import myPipelineParallelGrid, PipeDataParallelTopology
from deepspeed.runtime.state_dict_factory import SDLoaderFactory
from graphormer.utils.pretrained_layer_spec import PretrainedLayerSpec, LoraLayerSpec
...
class PipelineModule(nn.Module):
    ...
    def __init__(self,
        ...
        self._grid = myPipelineParallelGrid(process_group=self.world_group,
                                          topology=self._topo)
                ...
                for idx, layer in enumerate(self._layer_specs[start:stop]):
                    ...
                    if isinstance(layer, nn.Module):
                        ...
                        try:
                            ...
                            pass
                    print(f'    {idx+start:2d}: {name} -- {param_counts[start+idx]/1e6:.3f}M')
            if self.loss_fn:
                try:
    ...
    def fast_load_state_dir(self, checkpoint_engine, load_dir=None, strict=True, model_ckpt_list=None, mfm_lora=False):
        for idx, layer in enumerate(self.forward_funcs):
            # Functions, etc. will not have state_dicts
            if not hasattr(layer, 'load_state_dict'):
                continue
            ...
            # get all checkpoint files for the layer.
            model_ckpt = self.ckpt_layer_path_list(load_dir, idx, model_ckpt_list=model_ckpt_list)
            mp_rank = self._grid.get_slice_parallel_rank()
            mp_world_size = self._grid.get_slice_parallel_world_size()
            ...
            if ckpt_name.split('/')[-1] == 'model.hybrid_emb.pt' or ckpt_name.split('/')[-1] == 'layer_37-model_states.pt':
                strict = False
            else:
                strict = True
