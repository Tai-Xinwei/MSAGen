...
def process_param_groups(net, ndim, ndim2=None, d_tilde=1, mode='muT', mfm_lora=False, **kwargs):
    if mode == 'muT':
        param_groups = group_param(net, ndim, d_tilde, kwargs['lr'])
    elif mode == 'freezlamma':
        param_groups, t = group_param_copilot(net, ndim, ndim2, d_tilde, kwargs['lr'], mfm_lora=mfm_lora)
...
def myGroupAdam(net, impl=Adam, ndim=512, ndim2=512, d_tilde=1, mfm_lora=False, **kwargs):
    new_param_groups = []
    param_groups, t = process_param_groups(net, ndim, ndim2=ndim2, d_tilde=d_tilde, mode='freezlamma', mfm_lora=mfm_lora, **kwargs)
    for param_group in param_groups:
        new_param_groups.extend([param_group])
